...
class LaCrosse_TX23
{
    ...
    // trigger a read, then get the bit time in microseconds from the start frame
    unsigned int getBitTime(void);
	
	// reads sensor data, returns true if successfull
    // wind speed in m/s
    // wind direction in degrees 0-359
	// gust speed in m/s
    bool read(float &speed, uint16_t &direction, float &gustspeed);
    
    // reads raw sensor data, returns true if successfull
    // wind speed in 0.1 m/s
    // wind direction 0-15
	// gust speed in 0.1 m/s
	bool readRaw(uint16_t &speed, uint8_t &direction, uint16_t &gustspeed);
    ...
    struct RawData {
	  ...
	  uint16_t gustspeed; // last gust speed?
    };
